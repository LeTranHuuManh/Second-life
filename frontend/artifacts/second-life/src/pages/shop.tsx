...
import { MapPin, Star, Package, Clock, MessageCircle, ChevronRight } from "lucide-react";
...
const MOCK_SHOP = {
  ...
  rating: 4.8,
  ratingCount: 189,
  description: "Chuyên đồ gỗ thủ công, nội thất vintage chất lượng cao. Cam kết uy tín, hàng đúng mô tả.",
  responseRate: "98%",
        ...
        <div className="bg-white rounded-3xl border border-border shadow-sm p-6 -mt-8 relative z-10 mb-6">
          <div className="flex flex-col md:flex-row gap-6 items-start md:items-center">
            <Avatar className="h-20 w-20 ring-4 ring-white shadow-lg flex-shrink-0">
              ...
              <AvatarFallback className="bg-primary/20 text-primary text-xl">{MOCK_SHOP.name[0]}</AvatarFallback>
            </Avatar>
            <div className="flex-1 min-w-0">
              <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-4">
                <div>
                  <h1 className="text-2xl font-bold font-display">{MOCK_SHOP.name}</h1>
                  ...
                  <p className="mt-2 text-sm text-muted-foreground max-w-md">{MOCK_SHOP.description}</p>
                </div>
        ...
        {isLoading ? (
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-12">
            {Array.from({ length: 8 }).map((_, i) => (
              <div key={i} className="bg-white rounded-2xl p-3 border border-border">
