...
import {
  Star, MapPin, Store, MessageCircle, ShieldCheck, Info,
  ChevronRight, ChevronLeft, X, Play, ShoppingCart,
  Calendar, Package, Clock, AlertCircle, CheckCircle2, Loader2,
  ZoomIn,
} from "lucide-react";
    ...
    <>
      <div className="flex gap-2 mt-3 flex-wrap">
        {images.map((img, i) => (
          <button
            key={i}
            onClick={() => { setZoomIdx(i); setZoomOpen(true); }}
    ...
    <div className="relative">
      <div className="overflow-hidden" ref={emblaRef}>
        <div className="flex gap-4">
          {row1.map((p) => (
            <div key={p.id} className="flex flex-col gap-4" style={{ minWidth: "180px", maxWidth: "200px" }}>
    ...
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-md rounded-3xl p-0 overflow-hidden">
        <div className="bg-secondary/30 px-6 pt-6 pb-4">
          <DialogHeader>
            <DialogTitle className="text-xl font-display">Xác nhận mua hàng</DialogTitle>
        ...
        <div className="px-6 py-5 space-y-5">
          ...
          <div className="flex gap-3 items-center bg-secondary/20 rounded-2xl p-3">
            <img src={product.images[0]} className="w-14 h-14 rounded-xl object-cover" />
            <div className="flex-1 min-w-0">
              <p className="font-semibold text-sm line-clamp-2">{product.name}</p>
              <p className="text-primary font-bold mt-0.5">{formatPrice(product.buyPrice)}</p>
            </div>
          </div>
          ...
          <p className="text-xs text-muted-foreground text-right">Còn lại: {product.stock} sản phẩm</p>
          ...
          <div className="flex items-center justify-between bg-primary/5 border border-primary/20 rounded-2xl px-4 py-3">
            ...
            <span className="text-2xl font-bold text-primary">{formatPrice(product.buyPrice * qty)}</span>
          </div>
        </div>
...
function RentModal({
  ...
  const canValidate = startDate && endDate && new Date(endDate) > new Date(startDate);
    ...
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-md rounded-3xl p-0 overflow-hidden">
        <div className="bg-primary/5 px-6 pt-6 pb-4">
          <DialogHeader>
            <DialogTitle className="text-xl font-display">Đăng ký thuê</DialogTitle>
            <DialogDescription>Chọn thời gian và số lượng bạn muốn thuê</DialogDescription>
          </DialogHeader>
        </div>
        ...
        <div className="px-6 py-5 space-y-4">
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  value={endDate}
                  min={startDate || today}
                  onChange={(e) => { setEndDate(e.target.value); setValidResult(null); }}
                  className="w-full pl-9 pr-3 h-10 rounded-xl border border-border bg-white text-sm focus:border-primary focus:ring-2 focus:ring-primary/20 outline-none transition-all"
                />
              </div>
            </div>
          ...
          {/* Qty */}
          <div className="flex items-center justify-between">
            <span className="font-medium text-foreground text-sm">Số lượng</span>
            <div className="flex items-center gap-3 border border-border rounded-2xl overflow-hidden">
              <button
                onClick={() => setQty((q) => Math.max(1, q - 1))}
                disabled={qty <= 1}
          ...
          {validResult === false && (
            <div className="flex items-center gap-2.5 bg-red-50 border border-red-200 text-red-700 rounded-2xl px-4 py-3 text-sm font-medium">
              ...
              Sản phẩm không còn trống trong khoảng thời gian này. Vui lòng chọn ngày khác.
            </div>
          )}
          ...
          {days > 0 && (
            <div className="flex items-center justify-between bg-primary/5 border border-primary/20 rounded-2xl px-4 py-3">
              <span className="text-muted-foreground font-medium text-sm">Tổng cộng:</span>
...
export default function ProductDetail() {
  ...
  const handleRentConfirm = (start: Date, end: Date, qty: number) => {
    addToCart({ product: product!, type: "rent", quantity: qty, startDate: start, endDate: end });
    setIsRentModalOpen(false);
    toast({ title: "Đã thêm vào giỏ hàng để thuê", description: product!.name });
  };

  const handleAddToCart = () => {
    addToCart({ product: product!, type: product!.buyPrice ? "buy" : "rent", quantity: 1 });
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
          <div className="lg:col-span-5 space-y-3">
            ...
            <div className="flex gap-2">
              {[0, 1, 2].map((i) => <Skeleton key={i} className="w-16 h-16 rounded-xl" />)}
            </div>
          </div>
    ...
    <div className="w-full">
      <div className="container mx-auto px-4 py-6 max-w-6xl">
        ...
        <nav className="flex items-center gap-1.5 text-sm text-muted-foreground mb-6 flex-wrap">
          ...
          <Link href={`/tim-kiem?category=${encodeURIComponent(product.category)}`} className="hover:text-primary transition-colors">
            {product.category}
          </Link>
          <ChevronRight className="w-3.5 h-3.5" />
          ...
          <div className="lg:col-span-7 flex flex-col gap-5">
            ...
            <div className="flex flex-wrap items-center gap-4 text-sm">
              ...
              <div className="flex items-center gap-1">
                ...
                <span className={`font-bold ${product.stock <= 2 ? "text-red-500" : "text-foreground"}`}>
                  {product.stock}
                </span>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {product.buyPrice && (
                <div className="bg-secondary/40 border border-secondary rounded-2xl p-4">
                  ...
                  <p className="text-2xl font-bold text-foreground">{formatPrice(product.buyPrice)}</p>
              ...
              {product.rentPricePerDay && (
                <div className="bg-primary/5 border border-primary/20 rounded-2xl p-4">
                  ...
                  <div className="flex items-baseline gap-1">
                    <p className="text-2xl font-bold text-primary">{formatPrice(product.rentPricePerDay)}</p>
            ...
            <div className="bg-white border border-border rounded-3xl p-5 shadow-sm">
              ...
              <div className="flex gap-3 mt-4 pt-4 border-t border-border/60">
                ...
                <Link href={`/cua-hang/${product.shop.id}`} className="flex-1">
                  <Button variant="secondary" size="sm" className="w-full rounded-xl gap-2">
                  ...
                  {[5, 4, 3, 2, 1].map((star) => {
                    ...
                    return (
                      <div key={star} className="flex items-center gap-2 text-xs">
                        ...
                        <span className="text-muted-foreground w-7">{pct}%</span>
                      </div>
                    );
                  })}
                </div>
              ...
              <div className="space-y-6">
                {product.reviews.length === 0 ? (
                  ...
                  product.reviews.map((r: any) => (
                    <div key={r.id} className="pb-5 border-b border-border/40 last:border-0">
                      ...
                      <ReviewMedia images={REVIEW_IMAGES.slice(0, r.rating > 4 ? 2 : 0)} />
                      <p className="text-muted-foreground text-sm leading-relaxed mt-2">{r.comment}</p>
                    </div>
                  ))
                )}
              </div>
        ...
        {related2.length > 0 && (
          <div className="mb-10">
            <h3 className="text-xl font-bold font-display mb-5">Sản phẩm tương tự</h3>
